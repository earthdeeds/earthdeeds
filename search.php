...
?>
        	<div style="text-align:center;"><span class="style3">Project Search</span></div>
            <div style="text-align:center;"><span class="style27"><a href="wiser-info.php">Click here for more info about this search</a></span></div>
            <br />
	<div class="left3">
                <div class="box">
		 ...
		 <div class="right3"> 
								...
								if ($vartype) {
								$querymap = mysql_query("SELECT * FROM $tblsearch WHERE $keysearch LIKE '%$keyword%' || $keysearch1 LIKE '%$location%' "); 
								...
								if ($row["project_title"]){
														...
														$projectdesc = '<a class=\"usertext\" href=\"project-search.php?projid='.$projectid.'\">'.$projectname.'</a>';
...
$sig = md5('page'.$currpage.'phrase'.$keyword.'location'.$location.$secret);
$url = 'http://www.wiser.org/organizations/api_search?page='.$currpage.'&phrase='.$keyword.'&location='.urlencode($location).'&sig='.$sig.'&key='.$apiKey;
$ch = curl_init();
...
$long = 116.2413;
		...
		<?php echo 'Keyword: '.$keyword.' | Location: '.$location; ?>
		</br>
		<?php
		...
		if ($keyword && $location){
									$result = mysql_query("SELECT * FROM $tblproject where project_title LIKE '%$keyword%' || project_address1 LIKE '%$location%' ") 
					or die(mysql_error()); 					
				
					}
		if ($keyword && !$location){
							$result = mysql_query("SELECT * FROM $tblproject where project_title LIKE '%$keyword%'") 
					or die(mysql_error()); 					
				
		}
				if ($location && !$keyword){
							$result = mysql_query("SELECT * FROM $tblproject where project_address1 LIKE '%$location%' ") 
					or die(mysql_error()); 					
					
		}
if ($currpage < 2){
					while($row = mysql_fetch_array( $result ) or $row1 = mysql_fetch_array( $result1 ) or $row2 = mysql_fetch_array( $result2 )) {

								if ($row["project_title"]){
								$projtemp = $row['project_account_idnum'];
							$resultprojteam = mysql_query("SELECT * FROM  wp_cc_teamproject where project_id = '$projtemp'") or die(mysql_error()); 
							$rowprojteam = mysql_fetch_array( $resultprojteam );
								
echo'<div class="wiser-box">';


						echo '<div class="left-search"><img  style = "height: 50px; width: 50px;" src="images/earthdeeds.png" /></div>';
								
			echo'           		
			
			<div class="right-search">
			<a style="font-weight: bold;"> '.$row["project_title"].'</a>
			<div><span class="style8">'.strip_tags($row['project_description']).'.....</span></div>
            </div>
            <div><a style= margin: 10px 0 0 0; href="http://beta.earthdeeds.com/project-search.php?projid='.$row['project_account_idnum'].'&teamid='.$rowprojteam['team_id'].'"><input type = "submit" name="submit" value="Learn More"></div>
...
$apiKey = 'f9db17962e68feb90fdb466cd787775c';
$secret = 'e6c2545b2849c5f31927d1ee0a9438b8';
//$newlocation = str_replace(', ', '%2+', $location);
$sig = md5('page'.$currpage.'phrase'.$keyword.'location'.$location.$secret);
$url = 'http://www.wiser.org/organizations/api_search?page='.$currpage.'&phrase='.$keyword.'&location='.urlencode($location).'&sig='.$sig.'&key='.$apiKey;
...
foreach ($xml->results->organization as $element) {
	...
	echo'<div class="wiser-box">';
			...
			echo '<div class="left-search"><img src="images/wiser.png" /></div>';	
...
<br />';
	...
	echo'           		
            ...
            <div><input type = "submit" name="submit" value="Learn More"></div>
			</form>
...
//            logExc("<: s = ".$s."; c = ".$c); // debugging instrumentation 
           // $aPageButtons[++$p] = "<td><a href=\"?phrase=".$keyword."&location=".$location."&s=1&cursor=0\">&lt;</a></td>\r"; 
...
//        logExc("Prev: s = ".$s."; c = ".$c); // debugging instrumentation 
        $aPageButtons[++$p] = "<td><a href=\"?phrase=".$keyword."&location=".$location."&s=".$sx."&cursor=".$c."\">Prev</a></td>\r"; 
        $sPageButtons .= "\t\t<a href=\"?phrase=".$keyword."&location=".$location."&s=".$sx."&cursor=".$c."\">Prev</a>\r"; 
    } 
...
//            logExc($s.": s = ".$s."; c = ".$c); // debugging instrumentation 
            ...
            $aPageButtons[++$p] = "<td><a href=\"?phrase=".$keyword."&location=".$location."&s=".$s."&cursor=".$c."\">".$i."</a></td>\r"; 
            $sPageButtons .= "\t\t<a href=\"?phrase=".$keyword."&location=".$location."&s=".$s."&cursor=".$c."\">".$i."</a>\r"; 
        } 
    } 
...
//        logExc("Next: s = ".$s."; c = ".$c); // debugging instrumentation 
        $aPageButtons[++$p] = "<td><a href=\"?phrase=".$keyword."&location=".$location."&s=".$sx."&cursor=".$c."\">Next</a></td>\r"; 
        $sPageButtons .= "\t\t<a href=\"?phrase=".$keyword."&location=".$location."&s=".$sx."&cursor=".$c."\">Next</a>\r"; 
        if ($s < $iPages) // also generate right arrow button (New: simplified control structure) 
        { 
            $s = $iPages; 
            $c = ($s - 1) * $ROWCOUNT; 
//            logExc(">: s = ".$s."; c = ".$c); // debugging instrumentation 
           // $aPageButtons[++$p] = "<td><a href=\"?phrase=".$keyword."&location=".$location."&s=".$s."&cursor=".$c."\">&gt;</a></td>\r"; 
          //  $sPageButtons .= "\t\t<a href=\"?phrase=".$keyword."&location=".$location."&s=".$s."&cursor=".$c."\">&gt;</a>\r"; 
        } 
    } 
...
?>
            ...
            <div><input type = "submit" name="submit" value="Request project"></div>
          <div class="clear"></div>
        </div>
