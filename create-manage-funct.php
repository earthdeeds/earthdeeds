...
include('connect-db.php');			
		...
		if( get_magic_quotes_gpc() )
		{
		 ...
		 $phoneinput= mysql_real_escape_string( $phonehtml);
			...
			$resultfilteruser = mysql_query("SELECT * FROM $tbl where user_username = '$username' || user_facebook_id = '$username'")or die(mysql_error());  
			...
			$message = "This email is to confirm that you just registered as a user on the Earth Deeds website.\r\n\r\nYou can access your account by logging in at beta.earthdeeds.com\r\n\r\nThank you for registering.\r\n\r\nSincerely,\r\n\r\nThe Earth Deeds team";
...
if ($teamreg == "member"){	
	header("Location: http://beta.earthdeeds.com/confirm.php?teamid=$teamid&autologin=true&uname=$username"); 
	}
	else
	{
	//header("Location: http://beta.earthdeeds.com/mystuff.php"); 
	header("Location: http://beta.earthdeeds.com/confirm.php?user=registered&autologin=true&uname=$username"); 
	}
			 }
	}	
...
include('connect-db.php');
		...
		if( get_magic_quotes_gpc() )
		{
		 ...
		 $phoneinput= mysql_real_escape_string( $phonehtml);
				...
				if($rowCheck > 0){
				if ($filePath == "imagesprofiles/") {
				$result = mysql_query("UPDATE `shantiom_sandbeta`.`wp_cc_user_account_info` SET `user_first_name` = '$firstnameinput', `user_last_name` = '$lastnameinput', `user_email` = '$email', `user_address1` = '$add1input', `user_address2` = '$add2input', `user_city` = '$cityinput', `user_state` = '$stateinput', `user_zip` = '$zipinput', `user_phone` = '$phoneinput', `user_password` = '$password',`user_date` = now() WHERE `wp_cc_user_account_info`.`ID` ='$userid';") or die(mysql_error()); 
				
				 header("Location: http://beta.earthdeeds.com/mystuff.php"); 
				 }
				 else
				{
				$result = mysql_query("UPDATE `shantiom_sandbeta`.`wp_cc_user_account_info` SET `user_first_name` = '$firstnameinput', `user_last_name` = '$lastnameinput', `user_email` = '$email', `user_address1` = '$add1input', `user_address2` = '$add2input', `user_city` = '$cityinput', `user_state` = '$stateinput', `user_zip` = '$zipinput', `user_phone` = '$phoneinput',`user_profile_photo`='$filePath', `user_password` = '$password',`user_date` = now() WHERE `wp_cc_user_account_info`.`ID` ='$userid';") or die(mysql_error()); 
	...
	else
	{
		...
		header("Location: http://beta.earthdeeds.com/error.php"); 
	...
	$result = mysql_query("SELECT * FROM $tbl where user_username = '$user_username' || user_facebook_id = '$user_username'") or die(mysql_error());  
...
$team_carbonprice = 36;
	...
	$result = mysql_query("SELECT * FROM $tbl where user_username = '$team_initiator' || user_facebook_id = '$team_initiator'") or die(mysql_error());  
	...
	if($rowCheck > 0){
		...
		mysql_query("INSERT INTO `shantiom_sandbeta`.`wp_cc_team_account` (`Team_account_idnum`, `user_account_id`, `team_title`, `team_initiator`, `team_description`, `team_long_description`, `team_location`, `team_image`, `team_password`, `team_status`, `team_url`, `team_emissions`, `team_member_emissions`,`team_date`, `carbon_price`) VALUES (NULL, '$userid', '$team_titleinput', '$team_initiator', '$team_descriptioninput', '$team_long_descriptioninput', '$team_locationinput', '$filePath', '$team_password', '$team_status', '$team_urlinput', '$team_emissions', '$team_member_emissions',now(),'$team_carbonprice');") or die(mysql_error()); 
			
			$result = mysql_query("SELECT * FROM wp_cc_team_account ORDER BY Team_account_idnum DESC LIMIT 1") or die(mysql_error());  
					while($row = mysql_fetch_array( $result )) {
						$teamaddid = $row['Team_account_idnum'];
						}
		
			mysql_query("INSERT INTO `shantiom_sandbeta`.`wp_cc_team_manager` (`team_manger_id`, `team_manager_userid`, `team_id`, `manager_fname`) VALUES (NULL, '$userid', '$teamaddid', '$team_initiator');") or die(mysql_error());		
...
$filePath = addslashes($filePath);
	...
	$result = mysql_query("SELECT * FROM $tbl where user_username = '$projectusername' || user_facebook_id = '$projectusername'") or die(mysql_error());  
