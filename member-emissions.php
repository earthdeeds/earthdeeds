	...
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></script>
    		...
    		<?php	
				echo '</style>';
					...
					echo '
						<h1 class="entry-title" style = "font-size: 24px;">Lists of Member Emissions for '.$rowteam['team_title'].'</h1><br/>';
			...
			?>
				<script Language="JavaScript">
					...
					window.location.href = "member-emissions.php?teamid=" + teamid +"&sort=" + topcat ; 
					}
...
$orderlist = "date_added";
			...
			while($rowEmm = mysql_fetch_array( $result )) {
				...
				$res = mysql_query("SELECT * FROM $tbl where ID = '$userid'")or die(mysql_error()); 	
				while($rowuser = mysql_fetch_array( $res )){	
...
echo '<td>N/A</td>';
			...
			/*echo '<tr><td><font style="font-weight:bold" size="4">TOTALS</font></td>';
			echo '<td></td>';
			echo '<td></td>';
			echo '<td><strong>'.$tot_car_emm.'</strong></td>';
			echo '<td><strong>'.$tot_train_emm.'</strong></td>';
			echo '<td><strong>'.$tot_bus_emm.'</strong></td>';
			echo '<td><strong>'.$tot_plane_emm.'</strong></td>';
			echo '<td><strong>'.$tot.'</strong></td></tr>';*/
